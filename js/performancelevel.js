...
$().ready(function () {
    ...
    arr_page[0] = new Array("../../modules/market/performanceLevel.php", "单位层次列表", 0, '');
    var pages = "";
...
function getList() {
    $.post('../../modules/action/getPerformanceLevelList.php', {}, function (data) {
        $('#list').html(data);
    });
}
...
function deleteThis(obj) {
    if (window.confirm("你确定删除此记录吗？")) {
        $.post('../../modules/action/actionPerformanceLevel.php', {type: "delete", id: obj}, function (data) {
            getList();
        });
        return true;
    } else {
