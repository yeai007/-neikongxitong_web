...
if (isset($request_id) && $request_id > 0 && isset($request_type)) {
    if ($request_type == "see") {
        ...
        $result = $info->getInfo($request_id);
        $data["cert_info"] = $cert_info->getInfo($result["CertId"]);
        $data["student_info"] = $student_info->getInfo($data["cert_info"]["StudentId"]);
        $data["info"] = $result;
        $data["btn"] = "modify";
...
echo $twig->render('cert/set_certreceive.twig', $data);
